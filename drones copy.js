let show = true
    ...
    const container2 = d3.select('#box2');
    ...
    d3.csv('data.csv').then(function(data){
      ...
      // window.addEventListener('resize', wider);
      window.addEventListener('resize', widerOne)
// ............................................
      svg3
      ...
      function widerOne(){
        ...
        .attr('fill', 'red')
        .attr('cx', function(d,i){
          return 12 + (i * 20 * (window.innerWidth / 1500)) 
        })
        .attr('cy', 13 * (innerHeight / 850))
        .attr('r', function(d){
          return d.range * (window.innerWidth / 1400)
        })
        ...
        svg.selectAll('.rectangles')
          .attr('width', width / 80)
          .attr('x', function(d,i){
            return 9 + (i * 60 * (width / 1530))
          })
          // .attr('fill','blue')
          .attr('y',function(d,i){
            return svgHeight - barLength(d.st) - 102 + width/30
          })
        svg.selectAll('.text')
          .attr('x', function(d,i){
            return (i * 60 * (width / 1530))
          })
        ...
        svg.selectAll('.numbers')
          .attr('x',function(d,i){
            return 14 +(i * 60 * ( width / 1530))
          })
          .style('font-size', width / 80)
          .attr('y',function(d,i){
            return svgHeight - barLength(d.st) -112 + width/30
          })
        
        svg2.selectAll('.texted')
          .attr('x', function(d,i){
            return (i * 60 * (width / 1530))
          })
          .style('font-size', width / 80)
        ...
        svg2.selectAll('.rectangles')
          .attr('width', width / 80)
          .attr('x', function(d,i){
            return 12 + (i * 60 * (width / 1530))
          })
          // .attr('fill','blue')
          .attr('y',function(d,i){
            return svgHeight - barLength(d.killed) - 2 + width/30
          })
        svg2.selectAll('.numbers')
          .attr('x',function(d,i){
            return 16 +(i * 60 * ( width / 1530))
          })
          .style('font-size', width / 80)
          .attr('y',function(d,i){
            return svgHeight - barLength(d.killed) -10 + width/30
          })
